...
%load model
model = load('RedHumanSarah');%redHUMAN_recon2_smin.mat');

model=(model.RedModel);%redHUMAN_recon2_smin_02Sep2019_135437);

thermo_data=thermo_data_PATH

table_MTs=list_tasks_recon(model,thermo_data)

%%
function table_MTs = list_tasks_recon(model,thermo_data)
